...
$(document).ready(function() {
	...
	$('#strEmail').keyup(function(){
		var txt = $('#strEmail').val().replace(/(?:^|)\w/g, function(match) {
	...
	$('#deleteBtn').click(function() {
		...
		$.ajax({
			...
			url : "${pageContext.request.contextPath}/deleteEmail",
			data: "id="+deleteBinID,
			dataType : 'json',
			...
			success : function(data) {
				...
				if(data.outResponseCode == "00"){
					...
					setTimeout(function(){// wait for 5 secs(2)
						var obj = { Title: '', Url: 'emailConfigForm' };
	...
	var value = $("#emailConfig").validate({
		rules: 
		{
			strEmail: 
			{
		...
		submitHandler: function(form) 
		{
			if($('#strSelectID').val() == "N") 
			{
				var data = eval('${email}');
				var flag = false;
				var binVal = form.strEmail.value;
				$.each(data, function( index, value ) {
					if(binVal == JSON.stringify(value.strEmail).replace("\"","").replace("\"","")) {
						flag = true;
						return false;
					}
				});
					if(flag) 
					{
						$('#strError').text("Duplicate Email Type");
						$('#errorMsg').hide();
					}
					else 
					{
			...
			{
				emailData.strExt = form.strExt.value;
				$.ajax({
					type : "POST",
					contentType : "application/json",
					url : "${pageContext.request.contextPath}/updateEmail",
...
{     
    	...
    	$.each(data, function( index, value ) 
    	{
			if(bin == JSON.stringify(value.strEmail).replace("\"","").replace("\"","")) {
				emailData = value;
				return false;
			}
		});
    	$('#strEmail').val(emailData.strEmail);
    	$('#strEmail').attr('readonly', true);
    	$('#strExt').val(emailData.strExt);
    	$('#strSelectID').val("Y");
...
<div class="my-3 my-md-5">
	<div class="container">
		<div class="row">
			...
			<div class="col-md-9">
				<form:form action="emailConfig" method="post" commandName="emailTypeModel"
					name="emailConfig" id="emailConfig" cssClass="card">
					
					<div class="card-header">
						<h3 class="card-title"><i class="fe fe-user-plus"></i> &nbsp;&nbsp;Email Type Configuration <span class="text-right badge badge-danger" id="strError"></span></h3>
						...
						<div class="row">
							...
							<div class="col-md-6 col-lg-6">
								<div class="form-group">
									<label class="form-label">Email Type <b>*</b></label>
									<form:input path="strEmail" id="strEmail"
										cssClass="form-control" />
									<span class="error" id="strEmailError"><form:errors
											path="strEmail"></form:errors></span>
								</div>
							</div>
					...
					<div class="card-body">
					<table id="emailTable" class="table table-striped table-bordered nowrap" style="width:100%">
					<thead>
						<tr>
							<th>Email Type</th>
							<th>Ext</th>
					...
					<tbody>
						<c:forEach var="itr" items="${emailList}">
							<tr>
								<td>${itr.strEmail}</td>
								<td>${itr.strExt}</td>
								<td>
									<a class="icon" href="javascript:editFunction('${itr.strEmail}')">
						...
						<div class="modal-dialog modal-sm">
							<div class="modal-content">
								<div class="modal-header">
									<p class="text-capitalize">Delete Email Type <label id="delBinMsg"></label></p>
								</div> 
								<div class="modal-body">

									<form>
										<p class="leading-loose">Are you sure you want to delete this Email Type ?</p>
									</form>
								</div>
