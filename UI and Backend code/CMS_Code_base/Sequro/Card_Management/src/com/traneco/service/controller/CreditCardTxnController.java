...
@RestController
public class CreditCardTxnController 
{
	...
	//@CrossOrigin(origins = "http://103.11.153.212:8585",methods = RequestMethod.POST)
	@RequestMapping(value = "/creditcardTxn", method = RequestMethod.POST , produces = "application/json")
	public TxnReqRes creditcardTxn(@RequestBody Map<String,Object> loginData)
	{	
		...
		{	
			...
			amount = Utils.decimalFormat.format(Utils.stringToBigDecimal(amount));
				...
				if(cardDetailStatus.equals("A") ) //Card Status Check POINT
				{
					...
					if (currentDate.compareTo(expiryDate) < 0) //Expiry Date Check POINT
					{
						...
						if (cardaccAcntLinkageMaster!=null)
						{
							...
							processResponse = accountManagementService.sendCreditCardTransactionData(txnReqResponse);
						}
						else
						{
		...
		System.out.println("Prashant::::CreditCardTxnController.creditcardTxn()"+tranMaster.getStrAuthCode());
		...
		txnReqResponse.setResponse(processResponse);
		System.out.println("Prashant Response::::-----"+txnReqResponse);
