...
@RestController
public class TxnMultiCurrencyHandleController 
{
	@Autowired
	...
	@RequestMapping(value = "/doTransaction", method = RequestMethod.POST , produces = "application/json")
	public TxnReqRes doTransaction(@RequestBody Map<String,Object> loginData)
	{	
				...
				if(cardDetailStatus.equals("A") ) //Card Status Check POINT
				{
					...
					if (currentDate.compareTo(expiryDate) < 0) //Expiry Date Check POINT
					{
						...
						if (cardaccAcntLinkageMaster != null)
						{
							...
							//txnReqResponse = accountManagementService.sendTransactionData(txnReqResponse);
							processResponse = accountManagementService.sendMultiCurrencyTransactionData(txnReqResponse);
						}
						else
						{
